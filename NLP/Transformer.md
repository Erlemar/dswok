...
## Vanilla Transformer
The encoder consists of 6 identical layers, each with two sub-layers: a multi-head [[Attention#Self-Attention|self-attention]] and a position-wise feed-forward network, both followed by residual connections and layer normalization. All outputs are of dimension 512.
...
| Beam search | Maintain $k$ parallel candidate sequences ("beams"); at each step expand each beam and keep the top $k$ by cumulative log-probability. Produces more coherent text than greedy, but can still repeat. |
| Temperature sampling | Rescale logits by $T$ before softmax — $\text{softmax}(\text{logits}/T)$. $T > 1$ increases randomness, $T < 1$ decreases it; $T \to 0$ approaches greedy. Higher $T$ is useful for creative generation. |
| Top-K sampling | At each step, consider only the $K$ most probable tokens. Cheap way to cut off unrealistic tail choices. |
| Top-P (nucleus) sampling | Select the smallest set of tokens whose cumulative probability exceeds $P$. More adaptive than Top-K across contexts with different entropy. |
